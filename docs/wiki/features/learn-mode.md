...
1. 현재 설정으로 후보를 다시 만든다.
2. 세션 시작 시점에 후보를 셔플한다.
3. `wordCount`만큼 잘라 세션을 만든다.
4. 맞음/모름으로 현재 카드를 처리한다.
5. 모르는 카드는 `retryQueue`에 들어간다.
6. 현재 라운드가 끝나면 `retryQueue`만으로 다음 라운드를 만든다.
7. 더 이상 재시도 카드가 없으면 완료한다.
